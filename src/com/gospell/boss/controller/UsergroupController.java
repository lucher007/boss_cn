...
//		}
	...
	public String businessUnit(User form) {
		...
		if(user == null || user.getId() == null || "0".equals(user.getUsertype())){//未初始化用户，进入查询界面
			// 构建上级网络Map对象
			List<Network> list = networkDao.queryByList(new Network());
			Map<Integer, String> map = new HashMap<Integer, String>();
			for (Iterator iterator = list.iterator(); iterator.hasNext();) {
	...
	public String unitBusiness(Userbusiness form) throws IllegalAccessException, InvocationTargetException {
		Operator operator =  (Operator)getSession().getAttribute("Operator");
		User user = operator.getUser();
		...
		if("buyDevice".equals(form.getBusinesstype()) || "buyProduct".equals(form.getBusinesstype())){
			...
			if(buyingstblist != null && buyingstblist.size() > 0){
				Userbusinessdetail businessdetail = new Userbusinessdetail();
				...
				businessdetail.setStoreid(operator.getStoreid());
				businessdetail.setTerminalid(buyingstblist.get(0).getStbno());
				businessdetail.setTerminaltype("0");
				businessdetail.setStbno(buyingstblist.get(0).getStbno());
				...
				businessdetail.setContent(buyingstblist.get(0).getStbno());
				businessdetail.setPrice(buyingstblist.get(0).getPrice());
				businessdetail.setTotalmoney(buyingstblist.get(0).getPrice().multiply(new BigDecimal(buyingstblist.size())));
			...
			if(buyingcardlist != null && buyingcardlist.size() > 0){
				Userbusinessdetail businessdetail = new Userbusinessdetail();
				businessdetail.setNetid(user.getNetid());
				businessdetail.setOperatorid(operator.getId());
				businessdetail.setUserid(user.getId());
				businessdetail.setAreacode(user.getAreacode());
				businessdetail.setStoreid(operator.getStoreid());
				businessdetail.setTerminalid(buyingcardlist.get(0).getCardid());
				businessdetail.setTerminaltype("1");
				businessdetail.setStbno("");
				businessdetail.setCardid(buyingcardlist.get(0).getCardid());
				...
				businessdetail.setBusinesstypename(businesstype.getTypename());
				businessdetail.setFeename(businesstype.getFeename());
				businessdetail.setContent(buyingcardlist.get(0).getCardid());
				businessdetail.setPrice(buyingcardlist.get(0).getPrice());
				businessdetail.setTotalmoney(buyingcardlist.get(0).getPrice().multiply(new BigDecimal(buyingcardlist.size())));
			...
			if(buyingproductlist != null && buyingproductlist.size() > 0){
		...
		//停开机业务
			...
			businessdetail.setBusinesstypename(businesstype.getTypename());
			businessdetail.setFeename(businesstype.getFeename());
			businessdetail.setContent(terminalArr[0]);
		...
		//过户业务	
			...
			businessdetail.setBusinesstypename(businesstype.getTypename());
			businessdetail.setFeename(businesstype.getFeename());
			businessdetail.setContent("过户给新订户ID:"+user.getTransferuser().getId());
			businessdetail.setPrice(businesstype.getPrice()); 
			businessdetail.setTotalmoney(businesstype.getPrice()!=null?businesstype.getPrice():new BigDecimal(0));
		...
		//销户业务		
			...
			businessdetail.setBusinesstypename(businesstype.getTypename());
			businessdetail.setFeename(businesstype.getFeename());
			businessdetail.setContent(businesstype.getTypename());
			businessdetail.setPrice(businesstype.getPrice()); 
			businessdetail.setTotalmoney(businesstype.getPrice()!=null?businesstype.getPrice():new BigDecimal(0));
			totalmoney = totalmoney.add(businessdetail.getTotalmoney()==null?new BigDecimal("0"):businessdetail.getTotalmoney()); //累加过户的费用
			businessdetail.setAddtime(Tools.getCurrentTime());
		...
		//迁移业务		
			...
			businessdetail.setBusinesstypename(businesstype.getTypename());
			businessdetail.setFeename(businesstype.getFeename());
			businessdetail.setContent(getMessage("user.oldaddress")+":"+user.getAddress()+" 迁往 "+getMessage("user.newaddress")+":"+form.getAddress());
	...
	public String saveBusiness(Userbusiness form) {
		...
		if("buyDevice".equals(form.getBusinesstype()) || "buyProduct".equals(form.getBusinesstype())){
			...
			if(buyingcardlist != null && buyingcardlist.size() > 0){
				//保存订户智能卡
				usercardDao.saveBatch(buyingcardlist);
				//改变智能卡库存状态
				StringBuffer cardidsStrBuffer = new StringBuffer("");
				for(int i = 0;i<buyingcardlist.size();i++){
					if(i == buyingcardlist.size()-1){
						cardidsStrBuffer.append("'").append(buyingcardlist.get(i).getCardid()).append("'") ;
				    }else{
				    	cardidsStrBuffer.append("'").append(buyingcardlist.get(i).getCardid()).append("',") ;
				    }
				}
				...
				cardtmp.setState("1");
				cardtmp.setCardid(cardidsStrBuffer.toString());
				//批量修改智能卡状态
				cardDao.updateCardStateBatch(cardtmp);
				
				//批量发送智能卡开卡指令
				authorizeService.saveAuthorize_buyCard_batch(buyingcardlist);
			}
			...
			if(buyingproductlist != null && buyingproductlist.size() > 0){
				//批量保存产品
				userproductDao.saveBatch(buyingproductlist);
				//批量发送产品授权
				authorizeService.saveAuthorize_buyProduct_batch(buyingproductlist);
				//批量删除每个产品的过期授权记录
				for (Userproduct userproduct : buyingproductlist) {
	    ...
	    //停开机业务
			...
			if("pausecard".equals(form.getBusinesstypekey())){
				...
				if("0".equals(form.getTerminaltype())){//无卡机顶盒终端
					...
					usercardform.setCardids(cardidsStrBuffer.toString());
					usercardform.setState("0");//报停
					usercardDao.updateState_batch(usercardform);//保存修改
		...
		//过户业务	
			...
			//过户改变了区域，需要重新激活
			if(StringUtils.isNotEmpty(user.getAreacode()) && StringUtils.isNotEmpty(user.getTransferuser().getAreacode())){
	...
	public String saveRechargeAccount(User form){
		...
		Businesstype typeform = new Businesstype();
		typeform.setTypekey("rechargeaccount");
		Businesstype businesstype = businesstypeDao.findByTypekey(typeform);
        if(businesstype == null ){
        	businesstype = typeform;
        }
	...
	public String unitCancelBusiness(Userbusiness form) {
		...
		if("cancelproduct".equals(form.getBusinesstype())){//产品退订
			if(!"".equals(form.getUserproductids()) && null != form.getUserproductids()){
				String[] userproductidArr = form.getUserproductids().split(",");
				List<Userproduct> canceluserproductlist = userproductDao.findByIdBatch(userproductidArr);
				//产品费用,按照同一个产品ID封装一条记录明细
				if(canceluserproductlist != null && canceluserproductlist.size() > 0){
					HashMap<String,Userproduct>  userproductMap = new HashMap<String,Userproduct>();
					for (Userproduct canceluserproduct : canceluserproductlist) {
						if(userproductMap.containsKey(canceluserproduct.getProductid())){
							Userproduct userproductDMP = userproductMap.get(canceluserproduct.getProductid());
							BigDecimal totalmoneydmp = userproductDMP.getTotalmoney().add(canceluserproduct.getTotalmoney());
							userproductDMP.setTotalmoney(totalmoneydmp);
							userproductMap.put(userproductDMP.getProductid(), userproductDMP);
						}else{
							Userproduct userproductDMP = new Userproduct();
							Tools.setVOToVO(canceluserproduct, userproductDMP);
							userproductMap.put(userproductDMP.getProductid(), userproductDMP);
						}
					}
			...
			businessdetail.setTotalmoney(totalmoney);
		...
		form.setUserbusiness(userbusiness);
		return "usergroup/feeCountCancel";
	}
...
//		}
		...
		if("cancelproduct".equals(form.getBusinesstype())){
			if(!"".equals(form.getUserproductids()) && null != form.getUserproductids()){
				String[] userproductidArr = form.getUserproductids().split(",");
				List<Userproduct> userproductlist = userproductDao.findByIdBatch(userproductidArr);
				if(userproductlist != null && userproductlist.size() > 0){
					//批量发送反授权指令
					for (Userproduct userproduct : userproductlist) {
		...
		//设备回收
			...
			if("1".equals(form.getTerminaltype())){//智能卡回收
				String[] terminalidArr = form.getTerminalids().split(",");
				List<Usercard> rebackUsercardlist = usercardDao.findByCardidStrBatch(terminalidArr);
				...
				if(rebackUsercardlist != null && rebackUsercardlist.size() > 0){
					...
					Card cardtmp = new Card();
					cardtmp.setState(form.getDevicestate());
					cardtmp.setCardid(cardidsStrBuffer.toString());
					//批量修改智能卡状态
					cardDao.updateCardStateBatch(cardtmp);
		...
		//清空收取的其他费用
		user.setBuyingotherfeelist(null);
	...
	public String getBuyingStbInfo(User form) {
		...
		return "usergroup/buyingStbInfo";
	}
	...
	public String findStbListForDialog(Stb form) {
		...
		return "usergroup/findStbListForDialog";
	}
	...
	public String getBuyingCardInfo(User form) {
		...
		//封装订户信息
		form.setUser(user);
		return "usergroup/buyingCardInfo";
	}
	...
	public String findCardListForDialog(Card form) {
		...
		return "usergroup/findCardListForDialog";
	}
	...
	public Map<String,Object> addBuyingCard(Card form) {
		...
		List<Usercard> buyingcardlist = user.getBuyingcardlist();
		//临时数组，用来保存购买中的智能卡号
		List<String> dmpBuyingcardidList = new ArrayList<String>();
		for (Usercard usercard : buyingcardlist) {
			dmpBuyingcardidList.add(usercard.getCardid());
		}
		
		//开始卡号
		Long startcardid = form.getStartcardid();
		//结束卡号
		Long endcardid = form.getEndcardid();
		
		if(endcardid < startcardid){
			responseJson.put("flag", "0");//选择智能卡失败
			responseJson.put("error", getMessage("结束卡号不能小于开始卡号"));
			return responseJson;
		}
		
		Long cardnum = endcardid-startcardid;
		if(cardnum > 100){
			responseJson.put("flag", "0");//选择智能卡失败
			responseJson.put("error", getMessage("一次性购买智能卡数量不能超过100张"));
			return responseJson;
		}
		
		form.setNetid(user.getNetid());
		List<Card> cardlist = cardDao.queryCardlistByStartcardidAndEndcardid(form);
		
		for (Card card : cardlist) {
				if(!dmpBuyingcardidList.contains(card.getCardid())){//智能卡号不在购买列表中
					Usercard usercard = new Usercard();
					usercard.setNetid(card.getNetid());
					usercard.setAreacode(user.getAreacode());
					usercard.setUserid(user.getId());
					usercard.setServerid(card.getServerid());
					usercard.setCardid(card.getCardid());
					...
					//默认为母机
					usercard.setMothercardflag(form.getMothercardflag());
					usercard.setCardflag(form.getCardflag());
					if("0".equals(form.getMothercardflag())){//母卡
						usercard.setPrice(card.getMainprice());
					}else{
		...
		if(buyingcardlist == null || buyingcardlist.size() < 1){
			responseJson.put("flag", "0");//选择智能卡失败
			responseJson.put("error", getMessage("库存中未找到该卡号段的未出库的智能卡"));
			return responseJson;
		}
	...
	public Map<String, Object> getBuyingCardJson(Userlevel form) {
		//封装JSon的Map
		...
		User user = operator.getUser();
		List<Usercard> buyingcardlist = user.getBuyingcardlist();
		Integer total = buyingcardlist.size();
		
		for (Usercard usercard : buyingcardlist) {
			HashMap<String, Object> objectMap = new HashMap<String, Object>();
			//产品信息
			objectMap.put("id", usercard.getCardid());
			objectMap.put("cardid", usercard.getCardid());
			objectMap.put("mothercardflag", usercard.getMothercardflag());
			objectMap.put("mothercardid", usercard.getMothercardid());
			objectMap.put("price", usercard.getPrice());
			objectMap.put("cardflag", usercard.getCardflag());
	...
	public String getBuyingProductInfo(User form) {
		...
		//封装订户信息
		form.setUser(user);
		return "usergroup/buyingProductInfo";
	}
	...
	public String findProductListForDialog(Product form) {
		...
		form.setCardid(cardidArr[0]);//选择第一个智能卡号作为代表，去查找该批次智能卡号购买产品的最后时间
		Operator operator =  (Operator)getSession().getAttribute("Operator");
		User user = operator.getUser();
		if("".equals(user.getUserlevelid()) || null == user.getUserlevelid()){
			...
			return "usergroup/findProductListForDialog";
		}
		...
		userlevelproductform.setProductid(form.getQueryproductid());
		userlevelproductform.setProductname(form.getQueryproductname());
		userlevelproductform.setPager_openset(5);
		userlevelproductform.setPager_count(userlevelproductDao.findeffectiveproductCount(userlevelproductform));
	...
	public Map<String,Object> addBuyingProduct(Product form) {
		...
		if("".equals(user.getUserlevelid()) || null == user.getUserlevelid()){
			...
			product.setNetid(user.getNetid());
			product.setProductid(userlevelproduct.getProductid());
			product.setProductname(userlevelproduct.getProductname());
			product.setPricepermonth(userlevelproduct.getPricepermonth());
			product.setPriceperday(userlevelproduct.getPriceperday());
		...
		String cardids = form.getCardids();
		String[] cardidArr = cardids.split(",");
		Card card = cardDao.findByCardidStr(cardidArr[0]);
		//所属服务器取第一张智能卡为代表
		Integer serverid = card.getServerid();
		String termainaltype = "1";
		...
		for(int i = 0;i<cardidArr.length;i++){
			if(i == cardidArr.length-1){
				cardidsStrBuffer.append("'").append(cardidArr[i]).append("'") ;
		    }else{
		    	cardidsStrBuffer.append("'").append(cardidArr[i]).append("',") ;
		    }
		}
		String terminalids = cardidsStrBuffer.toString();
		
		//先判断购买的产品是否在购买列表中
		List<Userproduct> buyingproductlist = user.getBuyingproductlist();
		for(String cardid : cardidArr){
			for (Userproduct userproduct : buyingproductlist) {
				if(cardid.equals(userproduct.getTerminalid()) && form.getProductid().equals(userproduct.getProductid())){
		...
		Userproduct userproductForm = new Userproduct();
		userproductForm.setUserid(user.getId());
		userproductForm.setTerminalids(terminalids);
		...
		//封装购买的订户产品LIST
		List<Userproduct> buyinguserproductlistDMP = new ArrayList<Userproduct>();
		...
		for(String cardid : cardidArr){
			...
			//获取购买产品的授权开始时间
			String buyingStartTime = "";
			if(userproductHP != null ){//该智能卡已经购买了一些产品，
				//获取该智能卡已经购买的该产品的数据库最大授权时间
				String buyedlastEndTime = userproductHP.get(form.getProductid());
				if(StringUtils.isNotEmpty(buyedlastEndTime)){//该智能卡已经购买了该产品，找出该智能卡购买该产品的最大授权时间
					//数据库最大的授权时间与当前时间进行比较
					int flag = Tools.compare_date(buyedlastEndTime,Tools.getCurrentTimeByFormat("yyyy-MM-dd"),"yyyy-MM-dd");
					if(flag >= 0 ){//表示数据库购买时间未过期，即该产品的开始购买时间应该为数据库未过期的最大授权时间+1
						buyingStartTime = Tools.getSpecifiedDayAfter(buyedlastEndTime,"yyyy-MM-dd",1);
					}
				}
			}
			//判断在数据库中获取的最大时间是否为空，如果为空，即表示还未购买该产品，该产品的开始时间为今天
			if(StringUtils.isEmpty(buyingStartTime)){
				buyingStartTime = Tools.getCurrentTimeByFormat("yyyy-MM-dd");
			}
			
			//获取该产品的购买结束时间
			String  buyingEndTime = "";
			if("day".equals(form.getUnit())){//按天购买，直接加上（购买天数-1）
				buyingEndTime = Tools.getSpecifiedDayAfter(buyingStartTime,"yyyy-MM-dd",form.getBuyamount()-1);
			}else{//按月购买，先加上月数，在减一天
				buyingEndTime = Tools.getSpecifiedMonthAfter(buyingStartTime,"yyyy-MM-dd",form.getBuyamount());
				buyingEndTime = Tools.getSpecifiedDayAfter(buyingEndTime,"yyyy-MM-dd",-1);
			}
			...
			if(!"1".equals(form.getProductid())){
				...
				for(Userproduct userproduct : buyingproductlist){
					if(cardid.equals(userproduct.getTerminalid()) && "1".equals(userproduct.getProductid())){
				...
				if(StringUtils.isNotEmpty(buyedEndtime_base)){//该智能卡已经购买了该产品
					//基本包的最大授权时间与当前付费产品的购买到期时间进行比较
					int flag = Tools.compare_date(buyedEndtime_base,buyingEndTime,"yyyy-MM-dd");
					if(flag < 0 ){//表示该产品的购买时间超过了基本包的授权时间
						responseJson.put("flag", "0");//选择产品失败
						responseJson.put("error", getMessage("智能卡:"+cardid+"不能购买， 购买时间超过了基本包的授权时间"));
						return responseJson;
					}
				}else{//未找到基本包的授权
					responseJson.put("flag", "0");//选择产品失败
					responseJson.put("error", getMessage("智能卡:"+cardid+"不能购买， 该智能卡还未购买基本包"));
					return responseJson;
				}
			}
			...
			userproduct.setTerminalid(cardid);
			userproduct.setTerminaltype(termainaltype);
			userproduct.setCardid(cardid);
			...
			userproduct.setStarttime(buyingStartTime + " 00:00:00");
			userproduct.setEndtime(buyingEndTime + " 23:59:59");
			...
			buyinguserproductlistDMP.add(userproduct);
		}
		
		
		//循环批量购买的智能卡
		for (Userproduct userproduct : buyinguserproductlistDMP) {
	...
	public Map<String, Object> getBuyingProductJson(Userlevel form) {
		//封装JSon的Map
		Map<String, Object> result = new HashMap<String, Object>(); 
		...
		User user = operator.getUser();
		List<Userproduct> buyingproductlist = user.getBuyingproductlist();
		Integer total = buyingproductlist.size();
		
		for (Userproduct userproduct : buyingproductlist) {
			HashMap<String, Object> objectMap = new HashMap<String, Object>();
			//产品信息
			objectMap.put("id", userproduct.getCardid());
			objectMap.put("cardid", userproduct.getCardid());
			objectMap.put("productid", userproduct.getProductid());
			objectMap.put("productname", userproduct.getProductname());
			objectMap.put("starttime", userproduct.getStarttime());
			objectMap.put("endtime", userproduct.getEndtime());
			objectMap.put("price", userproduct.getPrice());
			objectMap.put("buyamount", userproduct.getBuyamount());
			objectMap.put("totalmoney", userproduct.getTotalmoney());
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "/getUserCardJson")
	public Map<String, Object> getUserCardJson(Usercard form) {
		//封装JSon的Map
		Map<String, Object> result = new HashMap<String, Object>(); 
		...
		Usercard usercardform = new Usercard();
		usercardform.setUserid(user.getId());
		usercardform.setQuerystate("1");
		List<Usercard> usercardlist = usercardDao.queryByList(usercardform);
		if(usercardlist == null){
			usercardlist = new ArrayList<Usercard>();
		}
		
		String buytype = getRequest().getParameter("buytype");
		if("1".equals(buytype)){//未授权卡号
			Userproduct userproductform = new Userproduct();
			userproductform.setUserid(user.getId());
			HashMap<String,HashMap<String,String>> productLastEndTimeMp = userproductService.findAllProductLastEndTimeListByUserid(userproductform);
			HashMap<String,List<Usercard>> authorizecard = userproductService.findAuthorizecardFromAllcard(usercardlist, productLastEndTimeMp);
			List<Usercard> unAuthorizeCardlist = authorizecard.get("0");
			Integer total = unAuthorizeCardlist.size();
			for (Usercard usercard : unAuthorizeCardlist) {
			...
			result.put("rows", objectlist);
			return result;
		}else if("2".equals(buytype)){//单个卡号
			//页面输入的智能卡号
			String cardid = Tools.getStr(getRequest().getParameter("cardid"));
			List<Usercard> usercardlistdmp = new ArrayList<Usercard>();
			for (Usercard usercard : usercardlist) {
				if(cardid.equals(usercard.getCardid())){
					usercardlistdmp.add(usercard);
				}
			}
			//如果没找到，就自动把所有的智能卡赋值给页面
			if(usercardlistdmp.size() == 0){
				usercardlistdmp = usercardlist;
			}
			Integer total = usercardlistdmp.size();
			for (Usercard usercard : usercardlistdmp) {
				HashMap<String, Object> dataMap = new HashMap<String, Object>();
				//产品信息
				dataMap.put("id", usercard.getId());
				dataMap.put("cardid", usercard.getCardid());
			...
			Integer total = usercardlist.size();
			for (Usercard usercard : usercardlist) {
				HashMap<String, Object> dataMap = new HashMap<String, Object>();
				//产品信息
				dataMap.put("id", usercard.getId());
...
//		form.setUserlist(userlist);
		...
		return "usergroup/findTransferUserListForDialog";
	}
	...
	public String getReplaceDeviceInfo(User form) {
		...
		//封装订户信息
		form.setUser(user);
		return "usergroup/replaceDeviceInfo";
	}
	...
	public String findReplaceStbListForDialog(Stb form) {
		...
		return "usergroup/findReplaceStbListForDialog";
	}
...
//		}
	...
	public String findReplaceCardListForDialog(Card form) {
		...
		return "usergroup/findReplaceCardListForDialog";
	}
...
//		}
	...
	public String queryUserForDialog(User form) {
		...
		return "usergroup/queryUserForDialog";
	}
	...
	public String findByListForDialog(User form) {
		...
		return "usergroup/findUserListForDialog";
	}
    ...
    public HashMap<String,Object> updatesigncard(User form) {
		//封装返回给页面的json对象
