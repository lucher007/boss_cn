...
<body>
	<div class="form-box">
	  <form method="post" id="paramForm" name="paramForm">
	    <div class="cur-pos"><spring:message code="page.currentlocation" />：<spring:message code="menu.authorize.manage" />&gt;<spring:message code="menu.authorize.forcerestartstb"/></div>
	    <div class="fb-con">
	      <div class="cur-pos">
	            ...
	            <td align="right"><spring:message code="server.versiontype"/>：</td>
	             <td>
	             	<select name="versiontype" id="versiontype" class="select" onchange="changeVersiontype()">
		                <option value="gos_pn" <c:if test="${forcedrestart.versiontype == 'gos_pn' }">selected</c:if>><spring:message code="server.versiontype.gos_pn"/></option>
		                <option value="gos_gn" <c:if test="${forcedrestart.versiontype == 'gos_gn' }">selected</c:if>><spring:message code="server.versiontype.gos_gn"/></option>
		            </select>
	             </td>
	          </tr>
	        </table>
	      ...
	      <div id="info">
	      	<iframe id="forcedrestart" scrolling="auto" frameborder="0" style="width:100%;height:580px;"></iframe>
	      </div>
	       </form>
	    </div>
	    <!--
	...
	<script type="text/javascript">
		...
		function goBack(){
			$("#paramForm").attr("action", "cascommand/find_forcedrestart_List");
			$("#paramForm").submit();
		}
		...
		$(function() {
			cardid_optionInit();
			changeVersiontype();
			var returninfo = '${caspnforcedrestart.returninfo}';
			if (returninfo != '') {
		...
		function changeVersiontype() {
		    $("#forcedrestart").attr("src","<%=request.getContextPath()%>/cascommand/add_forcedrestart_Info?versiontype=" + $('#versiontype').val());
		}
