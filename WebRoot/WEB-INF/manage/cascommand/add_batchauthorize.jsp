...
<body>
	<div id="body">
			<div class="cur-pos"><spring:message code="page.currentlocation" />：<spring:message code="menu.authorize.manage" />&gt;<spring:message code="menu.authorize.batchauthorize"/></div>
			    ...
			    <tr height="40px">
					...
					<td align="left">
						<select id="querynetid" name="querynetid" class="select" onchange="queryTerminallist();">
							<c:forEach items="${product.networklist}" var="network" varStatus="s">
								<option value="${network.id}" <c:if test="${product.querynetid == network.id }">selected</c:if>>${network.netname}</option>
							</c:forEach>
						</select>
					</td>
			...
			<table width="100%">
				...
				<tr height="40px">
						<td align="center" colspan="6">
								<a href="javascript:preAuth();" class="easyui-linkbutton"><spring:message code="menu.authorize.batchauthorize"/></a>                                                                                                                                                                                  
						</td>
				</tr>
			</table>
	...
	function preAuth() {
		...
		if ($("#ids").val() == "") {
		 ...
		 $.dialog({
			    title: '<spring:message code="menu.authorize.batchauthorize"/>',
			    lock: true,
			    background: '#000', /* 背景色 */
			    opacity: 0.5,       /* 透明度 */
			    ...
			    icon: 'alert.gif',
			    ok: function () {
			    	$("#sendForm").attr("action", "cascommand/send_batchauthorize"+"?rid="+Math.random());
					$("#sendForm").submit();
		...
		//$("#sendForm").attr("action", "cascommand/send_batchauthorize").submit();
	}
	...
	function initTerminallist(){
            $('#terminallist').datagrid({
                 ...
                 url:'cascommand/queryUserterminalList',
                 pagination: true,
                 queryParams: {
					querynetid: $("#querynetid").val(),
