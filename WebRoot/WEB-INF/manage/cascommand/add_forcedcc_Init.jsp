...
<body>
	<div class="form-box">
	  <form method="post" id="paramForm" name="paramForm">
	    ...
	    <div class="fb-con">
	            ...
	            <td align="right"><spring:message code="server.versiontype"/>：</td>
	             <td>
	             	<select name="versiontype" id="versiontype" class="select" onchange="changeVersiontype()">
		                <option value="gos_pn" <c:if test="${caspnforcedcc.versiontype == 'gos_pn' }">selected</c:if>><spring:message code="server.versiontype.gos_pn"/></option>
		                <option value="gos_gn" <c:if test="${caspnforcedcc.versiontype == 'gos_gn' }">selected</c:if>><spring:message code="server.versiontype.gos_gn"/></option>
		            </select>
	             </td>
	          </tr>
	        </table>
	      ...
	      <div id="info">
	      	<iframe id="forcedcc" scrolling="auto" frameborder="0" style="width:100%;height:580px;"></iframe>
	      </div>
	       </form>
	    </div>
	    <!--
	...
	<script type="text/javascript">
		...
		function goBack(){
			$("#paramForm").attr("action", "cascommand/find_forcedcc_List");
			$("#paramForm").submit();
		}
		...
		function changeVersiontype() {
		    $("#forcedcc").attr("src","<%=request.getContextPath()%>/cascommand/add_forcedcc_Info?versiontype=" + $('#versiontype').val());
		}
