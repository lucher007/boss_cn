...
<body>
	<div id="body">
			...
			<form action="" method="post" id="searchForm" name="searchForm">
				<table width="100%">
						...
						<tr>
							  ...
							  <td><input type="text" id="querydate" name="querydate" value="${caspnnewemail.querydate}" onfocus="WdatePicker({lang:'${locale}',skin:'blueFresh',isShowWeek:true,isShowClear:true,dateFmt:'yyyy-MM-dd'});" class="Wdate inp w150" /></td>
		...
		<div class="lst-box">
			<table id="treetable" class="treetable" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr class="lb-tit">
					...
					<td><spring:message code="authorize.emailtype"/></td>
				...
				<c:forEach items="${caspnnewemail.casemaillist }" var="dataList">
					<tr height="30" class="lb-list">
						...
						<td>
						<c:choose>
	        				<c:when test="${dataList.cardid == 'ffffffff'}"><spring:message code="authorize.card.all"/></c:when>
	        				<c:otherwise>${dataList.cardid}</c:otherwise>
	        			</c:choose>
						</td>
						<td><spring:message code="authorize.emailtype.${dataList.emailtype}"/></td>
						<td>${dataList.emailtitle}</td>
						<td>${dataList.emailcontent}</td>
		...
		<div class="page">
			<cite><pg:pager url="cas_pn/find_Cmd35_List" items="${caspnnewemail.pager_count}" index="center" maxPageItems="10"
	...
	<script type="text/javascript">
		...
		function deleteById(id){
			...
			$.dialog.confirmMultiLanguage(
					...
					'<spring:message code="page.cancel"/>', function() {
						$("#searchForm").attr(
								"action",
								"cas_pn/send_cancel_Cmd35?id=" + id 
										+ "&pager_offset="
										+ '${caspnnewemail.pager_offset}'
										+ "&rid=" + Math.random());
						$("#searchForm").submit();
		...
		function query() {
			$("#searchForm").attr("action", "cas_pn/find_Cmd35_List");
			$("#searchForm").submit();
		}

		function add() {
			$("#searchForm").attr("action", "cas_pn/add_Cmd35_Init");
			$("#searchForm").submit();
		}
