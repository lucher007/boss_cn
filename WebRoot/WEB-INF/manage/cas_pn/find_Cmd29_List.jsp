...
<body>
	<div id="body">
		...
		<div class="lst-box">
			<table id="treetable" class="treetable" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr class="lb-tit">
					<td><spring:message code="osdid"/></td>
				
				<!-- 	<td><spring:message code="authorize.osd.keylock"/></td> -->
				
					<td><spring:message code="authorize.osd.showtimelen"/></td>
					<td><spring:message code="authorize.osd.showtimes"/></td>
					<td><spring:message code="authorize.osd.displaystyle"/></td>
				...
				<c:forEach items="${caspnforcedosd.caspnforcedosdlist }" var="dataList">
					<tr height="30" class="lb-list">
						...
						<td>${dataList.duration}</td>
						<td>${dataList.showtimes}</td>
						<td><spring:message code="authorize.forceosd.displaystyle.${dataList.style}"/></td>
		...
		<div class="page">
			<cite><pg:pager url="cas_pn/find_Cmd29_List" items="${caspnforcedosd.pager_count}" index="center" maxPageItems="10"
	...
	<script type="text/javascript">
		...
		function deleteById(id){
			...
			$.dialog.confirmMultiLanguage(
					...
					'<spring:message code="page.cancel"/>', function() {
						$("#searchForm").attr(
								"action",
								"cas_pn/send_cancel_Cmd29?id=" + id 
										+ "&pager_offset="
										+ '${caspnforcedosd.pager_offset}'
										+ "&rid=" + Math.random());
						$("#searchForm").submit();
		...
		function query() {
			$("#searchForm").attr("action", "cas_pn/find_Cmd29_List");
			$("#searchForm").submit();
		}

		function add() {
			$("#searchForm").attr("action", "cas_pn/add_Cmd29_Init");
			$("#searchForm").submit();
		}
