...
<body>
    ...
    <div class="seh-box">
        <form action="" method="post" id="searchForm" name="searchForm">
        	<input type="hidden" name="stbno" id="stbno" value="${stb.stbno}"/>
            <table width="100%">
				<tr>
					...
					<td width="130px" colspan="2">
						<input type="text" class="inp" name="servername" id="servername" readonly="readonly" style="background:#eeeeee;" value="${stb.server.servername }">
						<input type="hidden" name="queryserverid" id="queryserverid" value="${stb.server.id }"/>
					<!--
						 <select id="queryserverid" name="queryserverid" class="select" onchange="queryStb();">
							 <option value=""><spring:message code="page.select"/></option>
							 <c:forEach items="${stb.servermap}" var="servermap" varStatus="s">
			                   <option value="${servermap.key}" <c:if test="${stb.queryserverid == servermap.key }">selected</c:if>>${servermap.value}</option>
			                </c:forEach>
			             </select>
			        -->
					</td>
    ...
    <div class="lst-box">
    	<table id="treetable" class="treetable"  width="100%" border="0" cellspacing="0" cellpadding="0">
    		<tr class="lb-tit">
    			...
    			<td><spring:message code="stb.serverid"/></td>
    ...
    <div class="page">
    	<cite>
        	<pg:pager
			    url="user/findReplaceStbListForDialog"
...
<script type="text/javascript">
    ...
    //查询操作员
	function queryStb(){
		$("#searchForm").attr("action", "user/findReplaceStbListForDialog");
		$("#searchForm").submit();
	}	
  ...
  $(".lb-list").click(function(){
	var replacestbno = $(this).find("input[type=radio]").val();
	var data = {
	     stbno: '${stb.stbno}',
	     replacestbno: replacestbno
	   };
	var url="user/getReplaceStb?rid="+Math.random();
