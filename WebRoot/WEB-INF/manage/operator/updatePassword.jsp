...
<html>
	...
	<body>
	<div id="body">
		<div class="cur-pos"><spring:message code="page.currentlocation"/>：<spring:message code="menu.system.operator"/> &gt; <spring:message code="operator.passwordupdate"/></div>
		<form action="" method="post" id="searchform" name="searchform" autocomplete="off">
          ...
          <div class="fb-con">
		        <table>
		     	    <c:if test="${operator.remark == 'dialog'}">
			            <tr>
				            <td><spring:message code="operator.oldpassword"/>：</td>
				            <td>
				            	<input type="password" class="inp" name="oldPassword" id="oldPassword" >
				            </td>
						</tr>
					 </c:if>
					    <tr>
						...
						<tr>
						    ...
						    <td>
                   				<input type="password" class="inp" name="confirmPassword" id="confirmPassword"
                   				maxlength="15"
                   				>
                 				</td>
						</tr>
	...
	function checkVal() {
		if ("${operator.remark}" == "dialog") {
			if ($("#oldPassword") != undefined && $("#oldPassword").val() == "") {
				$.dialog.tips('<spring:message code="operator.oldpassword.empty"/>', 1, "alert.gif", function() {
					$("#oldPassword").focus();
				});
				return false;
			}
		...
		if($("#password") != undefined && $("#password").val() == ""){
			$.dialog.tips('<spring:message code="operator.newpassword.empty"/>', 1, "alert.gif", function() {
				$("#password").focus();
			});
			return false;
		}
